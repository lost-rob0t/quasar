...
(defun phase2-error-code-from-condition (thunk)
  (handler-case
      (progn (funcall thunk) nil)
    (quasar.protocol:quasar-error (condition)
      (quasar.protocol:quasar-error-code condition))))

(defun phase2-count-import-events ()
  (count-if
   (lambda (encoded)
     (search "\"event\":\"documents.imported\"" encoded))
   (car *events-box*)))

(defun test-phase2-revision-conflict-after-restart ()
                  ...
                  (let ((response
                    ...
                    (check
                     (string=
                      ...
                      (error-code response)))
                    (check (= 2 (quasar.store:direct-workspace-revision
                                 store-2 "default")))
                    (check
                     (string=
                      "document.not-found"
                    ...
                    (check (= 0 (phase2-count-import-events)))
                    (check
                     (notany
                      (lambda (entry)
                        (string=
                    ...
                    (let ((retry
                      ...
                      (check
                       (string=
                        ...
                        (error-code retry)))))
               (quasar.control-plane:unsubscribe plane-2 sub-id))))
      (when plane-1
        (ignore-errors
          (quasar.control-plane:stop-control-plane plane-1)))
      (when plane-2
...
(defun test-phase2-commit-replay-emits-once ()
         ...
         (let (session-id)
           (setf store-1
                 (quasar.store:make-tek9-store
                  ...
                  :failure-hook
                  (lambda (point)
                    (when (and (car armed) (eq point fault-point))
           ...
           (check
            (tek9-command-ok-p
             plane-1 "document.create" (make-doc "fault-base")
             :id "fault-base"))
           (setf session-id
                 (phase2-spec-session-id (phase2-spec-begin plane-1)))
           (check
           ...
           (multiple-value-setq (store-2 plane-2)
             (phase2-spec-reopen path))
           (check (= 1 (quasar.store:direct-workspace-revision
                        store-2 "default")))
...
(defun test-phase2-promotion-failure-matrix ()
  (dolist (point '(:before-import-promotion
                   :before-import-revision
                   :before-import-journal
                   :before-import-finalize))
    (phase2-run-promotion-fault-case point)))

(defun phase2-run-chunk-fault-case (fault-point)
  (let* ((path
           (unique-tek9-test-path
...
(defun test-phase2-chunk-failure-matrix ()
  (dolist (point '(:before-import-chunk
                   :after-import-documents
                   :before-import-metadata))
    (phase2-run-chunk-fault-case point)))

(defun phase2-page-document-ids (snapshot)
  (mapcar
   (lambda (document)
...
(defun test-phase2-pagination-property ()
               ...
               (loop until done
                     do
                        (let* ((snapshot
                               ...
                               (ids
                                 (phase2-page-document-ids snapshot))
                               (next
                                 (quasar.protocol:json-value
                                  page "nextOffset")))
                          (incf pages)
...
(defun test-phase2-expiry-and-workspace-isolation ()
  (with-temporary-tek9-store (store path "phase2-expiry")
    ...
    (check
     (string=
      "import.invalid-session"
      (phase2-error-code-from-condition
       (lambda ()
...
(defun phase2-stage-padding-chunk (start count padding-size)
  (apply
   #'quasar.protocol:json-array
...
(defun test-phase2-bounded-retained-memory ()
  (with-temporary-tek9-store (store path "phase2-memory")
    (declare (ignore path))
    (quasar.store:begin-import-stage
     store "memory" "stage" 0 100)
    (let ((sequence 0))
      (labels ((stage-range (start end)
                 (loop for first from start below end by 50
                       ...
                       do
                          (quasar.store:accept-import-chunk
                           ...
                           "stage"
                           sequence
                           (phase2-stage-padding-chunk
                            first count 2048)
                           (+ 100 sequence))
                          (incf sequence))))
        (stage-range 0 500)
        (phase2-force-full-gc)
        (let ((small (phase2-dynamic-usage)))
          (stage-range 500 5000)
          (phase2-force-full-gc)
          (let* ((large (phase2-dynamic-usage))
...
(defun run-phase2-adversarial-tests ()
  (let ((*failures* 0))
    (test-phase2-revision-conflict-after-restart)
    (test-phase2-commit-replay-emits-once)
    (test-phase2-promotion-failure-matrix)
    (test-phase2-chunk-failure-matrix)
    (test-phase2-pagination-property)
    (test-phase2-expiry-and-workspace-isolation)
    (test-phase2-bounded-retained-memory)
    (when (plusp *failures*)
      (error "~D Phase 2 adversarial tests failed." *failures*))
    t))