...
export default function AutoDigHostBridge() {
  ...
  const importHostDocuments = useCallback(
    async (documents) => {
      for (const id of documentIds(documents)) {
        pendingHostWrites.current.set(
          id,
          (pendingHostWrites.current.get(id) || 0) + 1
        );
      }
      ...
      try {
        ...
        savedIds = new Set(
          report.saved?.map((item) => item.id).filter(Boolean) || []
        );
        return report;
      } catch (error) {
        savedIds = new Set(
          error?.report?.saved?.map((item) => item.id).filter(Boolean) || []
        );
        throw error;
      } finally {
        for (const id of documentIds(documents)) {
    ...
    [bulkSaveDocuments, releaseHostWrite]
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = bridge.subscribe((event) => {
      ...
      if (event.type === "actor-findings" || event.type === "dataset-documents") {
        ...
        importHostDocuments(documents)
          .then((report) =>
            addDocumentsToActiveGraph(acceptedDocumentIds(report, documents))
          )
          .catch((error) =>
      ...
      if (
        ...
        typeof event.payload?.theme === "string"
      ) {
    ...
    bridge
      ...
      .then(async () => {
        ...
        if (!active) return;
        const documents = Array.isArray(dataset?.documents)
          ? dataset.documents
          : [];
        if (documents.length) {
  ...
  useEffect(() => {
    ...
    feed.on("change", (change) => {
      const bridge = bridgeRef.current;
      if (!bridge || change.id !== "workspace:default" || !activeGraph) return;
